...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   mutex_init(&pfifo->mutexFifo, NULL);
...
void insert_pfifo(PriorityFIFO* pfifo, int id, int priority) // so podemos inserir paciente se n estiver cheio
{
   require (pfifo != NULL, "NULL pointer to FIFO");  // a false value indicates a program error

   mutex_lock(&pfifo->mutexFifo);
   while(full_pfifo(pfifo)){
      cond_wait(&pfifo->notFull, &pfifo->mutexFifo);
   }
   ...
   mutex_unlock(&pfifo->mutexFifo);
}

/* --------------------------------------- */

// TODO point: synchronization changes may be required in this function
int retrieve_pfifo(PriorityFIFO* pfifo) // so podemos tirar se tivermos algo dentro da lista
{
   require (pfifo != NULL, "NULL pointer to FIFO");   // a false value indicates a program error
   ...
   while(empty_pfifo(pfifo)){
      cond_wait(&pfifo->notEmpty, &pfifo->mutexFifo);
   }
