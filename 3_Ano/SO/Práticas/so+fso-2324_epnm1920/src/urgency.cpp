...
void init_simulation(int np)
{
   ...
   for(int i = 0; i<np; i++){
      mutex_init(&hd->all_patients[i].mutexP, NULL);
...
void term_simulation(int np) {
   ...
   for(int i = 0; i<np; i++){
      ...
      mutex_destroy(&hd->all_patients[i].mutexP);
   }
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   mutex_lock(&hd->all_patients[patient].mutexP);
   ...
   mutex_unlock(&hd->all_patients[patient].mutexP);
   return 0;
}
...
{  
   mutex_lock(&hd->all_patients[id].mutexP);
   while(!hd->all_patients[id].done){
      cond_wait(&hd->all_patients[id].isTreated, &hd->all_patients[id].mutexP);
   }


   mutex_unlock(&hd->all_patients[id].mutexP);
...
void * nurse_life(void * args1){
   int * id = (int *)args1;
   while(1){
      int termination = nurse_iteration(*id);
      if(termination == 1){
         break;
      }
   }
...
void * doctor_life(void * args2){
   int * id = (int *)args2;
   while(1){
      int termination = doctor_iteration(*id);
      if(termination == 1){
         break;
      }
   }
...
int main(int argc, char *argv[])
{
   ...
   int args1[nnurses];
   for(int i = 0; i<nnurses; i++){ //criar as threads das nurses
      args1[i] = i;
      thread_create(&nurses[i], NULL, nurse_life, &args1[i]);
   }
   ...
   for(int i = 0; i<ndoctors; i++){
      args2[i] = i;
      thread_create(&doctors[i], NULL, doctor_life, &args2[i]);
   }

   pthread_t patient[npatients];
   int args3[npatients];
   for(int i = 0; i< npatients; i++){ // criar as threads dos pacientes
      args3[i] = i;
      thread_create(&patient[i], NULL, patient_life1, &args3[i]);
   }
   ...
   printf("All patients have been terminated\n");

   insert_pfifo(&hd->triage_queue,DUMMY_ID,1); //DUMMY ID PARA TERMINAR CICLOS
   ...
   printf("All nurses have been terminated\n");

   for(int i = 0; i<ndoctors; i++){
      thread_join(doctors[i], NULL);
   }
   printf("All doctors have been terminated\n");
