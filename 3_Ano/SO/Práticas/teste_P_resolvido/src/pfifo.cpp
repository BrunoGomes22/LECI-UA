...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   mutex_init(&pfifo->mutexo, NULL); //inicializar mutex

   cond_init(&pfifo->notFull, NULL); //inicializar var de cond
   
   cond_init(&pfifo->notEmpty, NULL); //inicializar var de cond
}
...
void term_pfifo(PriorityFIFO* pfifo)
{
   ...
   cond_destroy(&pfifo->notFull); // destruir vars de cond   
   cond_destroy(&pfifo->notEmpty);

   mutex_destroy(&pfifo->mutexo); // destruir mutex
}
...
{
   require (pfifo != NULL, "NULL pointer to FIFO");  // a false value indicates a program error
   mutex_lock(&pfifo->mutexo); // lock mutex 
   while(full_pfifo(pfifo)){ // nao podemos inserir nada enquanto estiver cheio (valor bool)
      cond_wait(&pfifo->notFull, &pfifo->mutexo); // fica a ouvir se manda um sinal para notFull
      // se alguma thread mandar um cond signal notFull acorda e verifica se esta cheio
   }      
   ...
   cond_broadcast(&pfifo->notEmpty); // sinalizar que nao esta vazio
   mutex_unlock(&pfifo->mutexo); // unlock mutex
}
...
{  
   require (pfifo != NULL, "NULL pointer to FIFO");   // a false value indicates a program error
   mutex_lock(&pfifo->mutexo); // lock mutex
   while(empty_pfifo(pfifo)){ // so podemos dar retrieve se n estiver vazio
      cond_wait(&pfifo->notEmpty, &pfifo->mutexo); // fica a ouvir se manda um sinal para notEmpty
   }
   ...
   cond_broadcast(&pfifo->notFull); // sinalizar que nao esta cheio (nao esta vazio)
   mutex_unlock(&pfifo->mutexo); // unlock mutex
   return result;
