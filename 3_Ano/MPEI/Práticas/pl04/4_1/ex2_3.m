...
for i = 1:length(tamanhos)
    ...
    for j = 1:length(chaves)
        hashCodes(j) = mod(string2hash(chaves{j}, 'sdbm'), length(vectorTables)) + 1; %faz o hashcode da chave j que já criámos

        if vectorTables(hashCodes(j)) > 0 %deteta se esse hashcode já está na hashtable
            collisions = collisions + 1;
        end

        vectorTables(hashCodes(j)) = vectorTables(hashCodes(j)) + 1; %"adiciona" o hashcode á hashtable
    end
