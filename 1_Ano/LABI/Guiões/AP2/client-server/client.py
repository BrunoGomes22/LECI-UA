...
def main():
	...
	while True:
		...
		if(data == "START"):
			data_to_send = { "op" : data, "client_id": sys.argv[1] }
		elif(data == "QUIT"):
			data_to_send = { "op" : data }
		elif(data == "GUESS"):
			value = int(input("Insira um valor de 0 a 100"))
			count += 1
			data_to_send = { "op" : data, "number" : value } 
		elif(data == "STOP"):
			data_to_send = { "op" : data, "number" : value, "attempts": count }
		...
		if (response["op"] == "START"):
			if(response["status"] == True):
				...
				print("Erro ao executar operação START (cliente existente).")
		elif (response["op"] == "QUIT"):
			if(response["status"] == True):
				print("Operação QUIT bem sucedida.")
			else:
				print("Erro ao executar operação QUIT (cliente inexistente).")
			...
			if(response["status"] == True):
				...
				print("Erro ao executar a operação GUESS (cliente inexistente).")
			...
			if(response["status"] == True):
				...
				print(f"Erro ao executar a operação STOP {response['error']}.")
