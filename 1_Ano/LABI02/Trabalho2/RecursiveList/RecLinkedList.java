...
public class RecLinkedList<T extends Comparable<T>>
{
	...
	*/
	public void insertLast (T e)
	{
		...
		if (first == null) first = newest; else last.next = newest;
		last = newest;
		size++;
	}
	...
	*/
	...
	private Node<T> insertPos (T e, int pos, Node<T> n) //n é o first
	{
	...
	*/
	...
	private RecLinkedList<T> copy (Node<T> n)
	{
		...
		/* implement this auxiliary function */
		if (n == null) {
			return new RecLinkedList<>();
		}
		final RecLinkedList<T> accumulator = copy(n.next);
		accumulator.insertFirst(n.elem);
		return accumulator;
	...
	*/
	...
	private RecLinkedList<T> reverse(Node<T> n)
	{
		...
		final RecLinkedList<T> accumulator = reverse(n.next);
		accumulator.insertLast(n.elem);
		return accumulator;
	}
	...
	*/   
	...
	{
		/* implement this auxiliary function */
		if(n.elem.equals(e)) return n.elem;
	...
	*/
	...
	private Node<T> removeAllElements (Node<T> n, T e)
	{
		...
		if(e.equals(n.elem)){
			size--;
			return removeAllElements(n.next, e);
		}
		n.next = removeAllElements(n.next, e);
		return n;
	...
	*/
	...
	private DelNode<T> removeFirstElement (Node<T> n, T e)
	{
		/* implement this auxiliary function */
		if(e.equals(n.elem)){
			DelNode<T> node = new DelNode<>(n, n.next);
			n.next = null; size--;
			return node;
		}
		DelNode<T> node = removeFirstElement(n.next,e);
	...
	*/
	...
	private int lastIndexOf (Node<T> n, T e, int p) throws NoSuchElementException
	{
		...
		pos = lastIndexOf(n.next, e, p+1);
		if(pos == -1 && n.elem.equals(e)){
			pos = p;
		}
		return pos;
	}
